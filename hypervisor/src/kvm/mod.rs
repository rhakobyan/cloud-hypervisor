...
#[cfg(target_arch = "x86_64")]
const VIRTUAL_ADDRESS_SIZE: u64 = 1 << 48;
#[cfg(feature = "sev_snp")]
use igvm_defs::PAGE_SIZE_4K;
#[cfg(feature = "sev_snp")]
use kvm_bindings::{KVM_X86_SNP_VM, KVM_MEMORY_ATTRIBUTE_PRIVATE, kvm_memory_attributes, kvm_segment as Segment};
...
impl hypervisor::Hypervisor for KvmHypervisor {
    ...
    fn create_vm(&self, _config: HypervisorVmConfig) -> hypervisor::Result<Arc<dyn vm::Vm>> {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            let kvm_guest_memfd_supported = fd.check_extension(Cap::GuestMemfd);
        ...
        match self.fd.run() {
            Ok(run) => match run {
                ...
                VcpuExit::MemoryFault { flags, gpa, size } => {
                    info!("VcpuExit::MemoryFault: flags={flags:#x}, gpa={gpa:#x}, size={size:#x}");
